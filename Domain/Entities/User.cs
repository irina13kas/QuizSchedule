...
namespace Domain.Entities
{
    public class User: BaseEntity
    {
        ...
        public User(string login, string password, string name, UserRole role, string photoUrl = default, DateTime birthDay = default,  string vkUrl = default)
        {
            ...
            if (string.IsNullOrWhiteSpace(name))
                throw new ArgumentException("Имя обязателен", nameof(login));

            if (string.IsNullOrWhiteSpace(vkUrl))
                throw new ArgumentException("VK обязателен", nameof(password));
