...
{
    public class GetQuizmanFinesHistoryQueryHandler : IRequestHandler<GetQuizmanFinesHistoryQuery, QuizmanFinesHistoryResponse>
    {
        ...
        public GetQuizmanFinesHistoryQueryHandler(IQuizmanRepository quizemanRepository, IFineRepository fineRepository,
            ICurrentUserService currentUserService, IMapper mapper)
        {
            _quizemanRepository = quizemanRepository;
            _finesHistoryRepository = fineRepository;
            _currentUserService = currentUserService;
            _mapper = mapper;
        }

        public async Task<QuizmanFinesHistoryResponse> Handle(GetQuizmanFinesHistoryQuery request, CancellationToken cancellationToken)
        {
            var userId = _currentUserService.UserId
                ?? throw new ForbiddenException("Пользователь не авторизован");

            if (_currentUserService.Role != UserRole.Admin.ToString())
                throw new ForbiddenException("Это эндпоинт для Администратора");
            ...
            return new QuizmanFinesHistoryResponse
            {
                QuizmanId = quizeman.Id,
